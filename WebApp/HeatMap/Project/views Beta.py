...
# View to render homepage or base dashboard
def index(request):
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            if isinstance(data, list):  # Extra check
                logger.info(f"Loaded {len(data)} sensor entries from JSON.")
                return data
            else:
...
def get_sensor_data(request):
    ...
    if category in ["temperature", "humidity"]:
        ...
        logger.debug(f"Returning {len(filtered_data)} {category} points.")
    ...
    logger.debug(f"Returning full dataset with {len(all_data)} points.")
